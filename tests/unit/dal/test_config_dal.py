...
class TestConfigDal:
    ...
    def test_insert_configs(self, db_session, fake_config_data):
    ...
    def test_get_config(self, db_session, fake_config_data):
        config_dal = ConfigDal(db_session, None)
        config_id = fake_config_data["id"]
        results = config_dal.get_configs(config_id)
        assert len(results) == 1

    def test_update_dark_theme(self, db_session, fake_config_data):
        config_dal = ConfigDal(db_session, None)
        config_id = fake_config_data["id"]
        status = (True, False,)
        for st in status:
    ...
    def test_update_check_program_updates(self, db_session, fake_config_data):
        config_dal = ConfigDal(db_session, None)
        config_id = fake_config_data["id"]
        status = (True, False,)
        for st in status:
    ...
    def test_update_show_info_messages(self, db_session, fake_config_data):
        config_dal = ConfigDal(db_session, None)
        config_id = fake_config_data["id"]
        status = (True, False,)
        for st in status:
    ...
    def test_update_check_reshade_updates(self, db_session, fake_config_data):
        config_dal = ConfigDal(db_session, None)
        config_id = fake_config_data["id"]
        status = (True, False,)
        for st in status:
    ...
    def test_update_create_screenshots_folder(self, db_session, fake_config_data):
        config_dal = ConfigDal(db_session, None)
        config_id = fake_config_data["id"]
        status = (True, False,)
        for st in status:
    ...
    def test_update_reshade_version(self, db_session, fake_config_data):
        config_dal = ConfigDal(db_session, None)
        config_id = fake_config_data["id"]
    ...
    def test_update_program_version(self, db_session, fake_config_data):
        config_dal = ConfigDal(db_session, None)
        config_id = fake_config_data["id"]
        new_version = "99.99.99"
